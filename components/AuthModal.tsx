...
export default function AuthModal() {
  ...
  useEffect(() => { setMode(authModalMode); setError(""); }, [authModalMode]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setError("");
    if (!email.trim() || !password.trim()) { setError(a.fillAllFields); return; }
    if (mode === "signup" && !name.trim()) { setError(a.enterName); return; }
    setLoading(true);
    try {
  ...
  return (
    ...
    >
      ...
      >
        <button onClick={closeAuthModal} className="absolute top-5 right-5 text-[#1a1a1a8c] hover:text-[#1a1a1a] transition-colors">
          <X size={16} strokeWidth={2} />
        </button>

        <div className="flex items-center gap-2 mb-8">
          <div className="size-2 bg-[#2563eb]" />
          <span className="font-['Oswald'] text-base font-medium tracking-[2.88px] uppercase">Prose &amp; Co.</span>
        </div>
          ...
          {(["signin", "signup"] as const).map((m) => (
            <button
              ...
              className={[
                ...
                mode === m ? "border-b-2 border-b-[#2563eb] text-[#1a1a1a]" : "text-[#1a1a1a8c] hover:text-[#1a1a1a]",
              ].join(" ")}
            >
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          {mode === "signup" && (
            <div className="flex flex-col gap-1.5">
              <label className="font-['Oswald'] text-[10px] leading-normal tracking-[2.4px] uppercase text-[#1a1a1a8c]">{a.name}</label>
          ...
          <div className="flex flex-col gap-1.5">
            <label className="font-['Oswald'] text-[10px] leading-normal tracking-[2.4px] uppercase text-[#1a1a1a8c]">{a.email}</label>
            <input ref={emailRef} type="email" value={email} onChange={(e) => setEmail(e.target.value)}
              placeholder={a.emailPlaceholder}
              className="h-11 border border-[#1a1a1a26] px-3 bg-transparent text-[15px] outline-none focus:border-[#2563eb] transition-colors placeholder:text-[#1a1a1a4d] placeholder:italic" />
          </div>
          <div className="flex flex-col gap-1.5">
            <label className="font-['Oswald'] text-[10px] leading-normal tracking-[2.4px] uppercase text-[#1a1a1a8c]">{a.password}</label>
          ...
          {error && <p className="text-[13px] italic text-[#1a1a1a8c]">{error}</p>}
          <button type="submit" disabled={loading}
            ...
            <span className="font-['Oswald'] text-[13px] leading-normal tracking-[2.86px] uppercase text-[#f7f6f3]">
              {loading ? a.loading : mode === "signin" ? a.signIn : a.createAccount}
            </span>
        ...
        <div className="flex items-center gap-3 mt-5">
          ...
          <span className="font-['Oswald'] text-[9px] tracking-[2px] uppercase text-[#1a1a1a4d]">{a.or}</span>
          <div className="flex-1 h-px bg-[#1a1a1a1a]" />
        </div>
        ...
        {mode === "signup" && (
          <p className="mt-5 text-[#1a1a1a8c] text-[12px] italic leading-normal">{a.trialNote}</p>
        )}

        <p className="mt-4 text-[#1a1a1a8c] text-[12px] leading-normal">
          {mode === "signin" ? (
            <>{a.noAccount}{" "}<button onClick={() => setMode("signup")} className="underline hover:text-[#2563eb] transition-colors">{a.createOne}</button></>
          ) : (
            <>{a.alreadySubscriber}{" "}<button onClick={() => setMode("signin")} className="underline hover:text-[#2563eb] transition-colors">{a.signInLink}</button></>
          )}
        </p>
      </div>
    </div>
