...
export default function EditorialPage() {
  ...
  return (
    <div className="flex flex-col bg-[#f7f6f3] text-[#1a1a1a] overflow-clip">
      ...
      {/* ── Hero ─────────────────────────────────────── */}
      <section className="flex relative w-full flex-col pt-8 md:pt-10 pb-10 md:pb-16 px-4 md:px-12 gap-7 overflow-clip">
        ...
        {/* Cover image */}
        <FadeIn delay={0.1}>
          <div className="relative w-full h-[220px] sm:h-[380px] md:h-[560px] bg-[#1a1a1a] overflow-clip">
            <div
            ...
            <div className="absolute flex items-center z-20 left-4 md:left-8 bottom-4 md:bottom-6 gap-2.5">
        ...
        {/* Hero headline */}
        <div className="flex flex-col font-['Oswald'] text-[clamp(52px,12vw,180px)] font-medium leading-[0.92] tracking-[-3.6px] uppercase overflow-hidden">
          <HeroLine delay={0.15}>{h.heroLine1}</HeroLine>
          <HeroLine delay={0.28}>{h.heroLine2}</HeroLine>
        </div>

        <div className="flex flex-col md:flex-row w-full justify-between items-start md:items-end gap-8 md:gap-12">
          <FadeIn delay={0.35} className="flex max-w-full md:max-w-[520px] flex-col gap-[18px]">
            <p className="text-[#1a1a1ad9] text-[17px] md:text-[19px] leading-[1.55]">
              {h.tagline}
            </p>
            <div className="flex flex-wrap items-center mt-1.5 gap-4 md:gap-6">
              <Link
                href="/editor"
                className="group flex h-[48px] md:h-[52px] items-center bg-[#2563eb] px-[18px] md:px-[22px] gap-2.5 hover:bg-[#1d4ed8] transition-colors"
              >
          ...
          <FadeIn delay={0.45} className="flex items-end pb-1 gap-8 md:gap-10">
            <div className="flex flex-col gap-1">
              <span className="font-['Oswald'] text-[36px] md:text-[44px] leading-none">
      ...
      <section className="flex flex-col md:flex-row w-full border-t border-b px-4 md:px-12 py-12 md:py-[88px] gap-8 md:gap-16 border-y-[#1a1a1a26]">
        <SlideIn direction="left" className="flex w-full md:w-[220px] flex-col shrink-0 gap-3">
          <div className="flex w-7 h-0.5 bg-[#2563eb]" />
          <span className="font-['Oswald'] text-[11px] leading-normal tracking-[3.08px] uppercase">
            {h.briefManifesto}
          </span>
        ...
        <FadeIn className="flex flex-col flex-1 gap-5">
          <div className="flex flex-wrap gap-x-4 font-['Oswald'] text-[clamp(28px,5vw,64px)] leading-[0.98] tracking-[-0.96px] uppercase">
            <span>{h.manifestoQuote}</span>
          </div>
          <div className="flex flex-col sm:flex-row items-start sm:items-center mt-2 gap-3 sm:gap-4">
            <span className="text-[#1a1a1a99] font-['Oswald'] text-[11px] leading-normal tracking-[2.64px] uppercase shrink-0">
              § 01.
            </span>
      ...
      {/* ── Three desks ──────────────────────────────── */}
      <section className="flex w-full flex-col px-4 md:px-12 py-12 md:py-[112px] gap-12">
        <FadeIn className="flex flex-col md:flex-row justify-between items-start md:items-end pb-8 border-b gap-6 md:gap-12 border-b-[#1a1a1a26]">
          <div className="flex max-w-full md:max-w-[760px] flex-col gap-4">
            <div className="flex items-center gap-3">
              <div className="flex w-7 h-0.5 bg-[#2563eb]" />
              <span className="font-['Oswald'] text-[11px] leading-normal tracking-[3.08px] uppercase">
                {h.bureauLabel}
            ...
            <div className="flex flex-col font-['Oswald'] text-[clamp(36px,7vw,96px)] leading-[0.92] tracking-[-1.92px] uppercase">
              <span>{h.bureauHeadline1}</span>
              <span>{h.bureauHeadline2}</span>
            </div>
          </div>
          <p className="max-w-full md:max-w-[360px] pb-0 md:pb-3 text-[#1a1a1abf] text-base leading-[1.6]">
            {h.bureauSubtitle}
          </p>
        </FadeIn>
        ...
        <Stagger className="flex flex-col md:flex-row">
          {h.desks.map((desk: { no: string; label: string; title: string; body: string; note: string }) => (
            <StaggerItem key={desk.no} className="flex-1">
              <HoverLift className="flex flex-col h-full p-6 md:p-9 gap-[18px] border-t md:border-t-0 md:border-l first:border-t-0 md:first:border-l-0 border-[#1a1a1a1a] cursor-default">
                ...
                <h3 className="mt-3 font-['Oswald'] text-[28px] md:text-[34px] leading-none tracking-[-0.34px] uppercase">
                  {desk.title}
                </h3>
      ...
      <section className="flex flex-col md:flex-row w-full bg-[#1a1a1a] text-[#f7f6f3] px-4 md:px-12 py-12 md:py-[112px] gap-8 md:gap-16">
        <SlideIn direction="left" className="relative w-full md:w-[440px] h-[280px] md:h-[520px] flex-col shrink-0 overflow-hidden">
          <div
            className="absolute inset-0 bg-cover bg-no-repeat grayscale"
            style={{
              backgroundImage:
        ...
        <FadeIn delay={0.15} className="flex flex-col justify-between flex-1 py-2 gap-8 md:gap-10">
          <div className="flex flex-col gap-5 md:gap-7">
            ...
            <div className="h-[40px] md:h-[60px] text-[#2563eb] text-[80px] md:text-[120px] leading-[0.6]">"</div>
            <p className="max-w-[760px] text-[28px] md:text-[42px] leading-[1.18] tracking-[-0.42px]">
              {h.pressQuote}
            </p>
          </div>
          <div className="flex flex-col md:flex-row justify-between items-start md:items-end pt-7 border-t gap-5 md:gap-8 border-t-[#f7f6f333]">
      ...
      {/* ── CTA ──────────────────────────────────────── */}
      <section className="flex w-full flex-col px-4 md:px-12 py-16 md:py-[128px] gap-10">
