...
function LedgerCell({ value }: { value: string | boolean }) {
  ...
  return <span className="font-['Oswald'] text-xs leading-normal tracking-[2.16px] uppercase">{value}</span>;
}
    ...
    <div className="flex flex-col bg-[#f7f6f3] text-[#1a1a1a]">
      ...
      <section className="flex w-full flex-col px-12 py-[88px] gap-7">
        ...
        <div className="flex flex-col font-['Oswald'] font-medium leading-[0.92] uppercase overflow-hidden">
          <HeroLine delay={0.05}><span className="block text-[clamp(64px,12vw,180px)] tracking-[-3.6px]">{p.heroLine1p}</span></HeroLine>
          <HeroLine delay={0.18}><span className="block text-[clamp(64px,12vw,180px)] tracking-[-3.6px]">{p.heroLine2p}</span></HeroLine>
        </div>
        <div className="flex justify-between items-end mt-2 gap-12">
          <p className="max-w-[640px] text-[#1a1a1ad9] text-[19px] leading-[1.55]">{p.heroTaglineFull}</p>
      ...
      <Stagger className="flex w-full border-t border-b border-y-[#1a1a1a33]">
        {p.plans.map((plan: { no: string; label: string; name: string; period: string; description: string; features: string[]; cta: string; badge?: string }, idx: number) => (
          <StaggerItem key={plan.no} className="flex-1">
            <HoverLift className={["relative flex flex-col h-full p-10 gap-5 border-l first:border-l-0 border-l-[#1a1a1a33]", idx === 1 ? "bg-[#1a1a1a] text-[#f7f6f3]" : ""].join(" ")}>
              ...
              <span className="font-['Oswald'] text-5xl tracking-[-0.48px] uppercase">{plan.name}</span>
              <div className="flex items-end pt-2 gap-1.5">
                <span className={["pb-3 font-['Oswald'] text-[22px] leading-normal", idx === 1 ? "text-[#f7f6f3a6]" : "text-[#1a1a1a99]"].join(" ")}>CHF</span>
                <span className="font-['Oswald'] text-[88px] leading-[0.9]">{planPrices[idx]}</span>
      ...
      <section className="flex w-full flex-col px-12 py-[112px] gap-8">
        <div className="flex justify-between items-end pb-7 border-b gap-12 border-b-[#1a1a1a33]">
          <div className="flex flex-col gap-3.5">
            ...
            <span className="font-['Oswald'] text-7xl leading-[0.94] tracking-[-1.44px] uppercase">{p.ledgerHeadline}</span>
          </div>
          <p className="max-w-[320px] pb-2 text-[#1a1a1aa6] text-[15px] italic leading-normal">{p.ledgerSub}</p>
        </div>
        <div className="flex flex-col">
          {ledgerRows.map((row) => (
            <div key={row.feature} className="flex items-center py-[18px] border-b border-b-[#1a1a1a1a]">
              <span className="text-[15px] leading-normal flex-1">{row.feature}</span>
              {[row.monthly, row.annual, row.patron].map((val, i) => (
                <div key={i} className="flex w-[200px] justify-center items-center py-1.5"><LedgerCell value={val} /></div>
              ))}
            </div>
          ))}
        </div>
        <div className="flex items-center pt-3 border-t-2 border-t-[#1a1a1a]">
          ...
          {p.plans.map((plan: { no: string; name: string }, i: number) => (
            <div key={plan.no} className={["flex w-[200px] justify-center py-2.5", i === 1 ? "bg-[#1a1a1a0d]" : ""].join(" ")}>
      ...
      <section className="flex w-full border-t px-12 py-[112px] gap-16 border-t-[#1a1a1a33]">
        <div className="flex w-[300px] flex-col shrink-0 gap-3.5">
          ...
          <span className="font-['Oswald'] text-[56px] leading-[0.94] tracking-[-1.12px] uppercase">{p.faqHeadline}</span>
        ...
        <Stagger className="flex flex-col flex-1">
          {p.faqs.map((faq: { n: string; q: string; a: string }) => (
            <StaggerItem key={faq.n}>
              <div className="flex border-t py-7 gap-8 border-t-[#1a1a1a26]">
                <span className="w-10 shrink-0 pt-1 text-[#2563eb] font-['Oswald'] text-xs leading-normal tracking-[2.88px] uppercase">{faq.n}</span>
                <div className="flex flex-col flex-1 gap-2.5">
                  <h4 className="font-['Oswald'] text-[22px] leading-[1.15] tracking-[-0.11px] uppercase">{faq.q}</h4>
